...
mongoClient.open(function (err, mongoClient){
	...
	db.collection('employees', {strict: true}, function (err, collection){
		if (err){
			console.log('The employees colleciotn doesnt exists');
			populateDB();
		}
	});
});
...
exports.findAll = function (req, res){
	...
	db.collection('employees', function (err, colleciton){
		if (name){
			colleciton.find({"fullName": new RegExp(name, "i")}).toArray(function (err, items){
				res.jsonp(items);
			});
		} else {
